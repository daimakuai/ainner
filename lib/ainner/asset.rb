...
module Ainner
  class Asset
    ...
    def relative_digest_path
      digest_path.gsub /#{Ainner.env.public_folder}/, ""
    end
    ...
    def wrappable?
      !!(self.javascript? and !Ainner.env.modules_ignored.include?(@path) or self.template?)
    end
    ...
    def logical_path
      @logical_path ||= @path.gsub(/^(#{Ainner.env.paths.join("|")})\/?/, "")
    end
  end
end
